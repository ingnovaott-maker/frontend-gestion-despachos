...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, ModalComponent, RegistroCorrectivoComponent, PaginatorComponent],
  ...
  template: `
    <div class="d-flex flex-column gap-3">
        ...
        <div class="card-body d-grid gap-3">
          ...
          <div class="d-flex justify-content-end">
            <div class="input-group input-group-sm" style="max-width: 320px;">
              ...
              <button type="button" class="btn-outline-brand btn-brand--sm" (click)="filtroDocs='';" [disabled]="!filtroDocs">
                Limpiar
              </button>
            </div>
          </div>
      ...
      <!-- Modal de historial -->
      <app-modal [open]="historialOpen()" title="Historial de mantenimiento correctivo" size="xl" (closed)="historialOpen.set(false)">
      ...
      <!-- Modal de registro -->
      <app-modal [open]="isRegistroOpen()" [title]="placaSeleccionada ? 'Nuevo mantenimiento correctivo — Placa: ' + placaSeleccionada : 'Nuevo mantenimiento correctivo'" size="xl" (closed)="isRegistroOpen.set(false)">
        <app-registro-correctivo [inModal]="true" [initialPlaca]="placaSeleccionada" [resetToken]="registroReset" [modalOpen]="isRegistroOpen()" (saved)="onRegistroSaved()" (closed)="isRegistroOpen.set(false)"></app-registro-correctivo>
      </app-modal>
    </div>
  `,
})
export class CorrectivosComponent {
  ...
  onFileChange(evt: Event) {
    ...
    this.archivos.guardarArchivo(file, 'sicov', vigilado).subscribe({
      next: (resp) => {
        this.api
          .guardarArchivo(this.TIPO_CORRECTIVO, resp.nombreAlmacenado, resp.nombreOriginalArchivo, resp.ruta, vigilado)
          .subscribe({
            next: () => {
    ...
    return [
      {
        ...
        nombres_responsable: 'Ing. Correctivo Uno',
        detalle_actividades: 'Diagnóstico de falla y reparación menor.'
      },
      {
        placa,
        fecha: d(20),
        ...
        tipo_identificacion: 'NIT',
        numero_identificacion: '800086050',
        nombres_responsable: 'Ing. Correctivo Dos',
        detalle_actividades: 'Reemplazo de componente y validación en ruta.'
      }
    ];
  }
}
