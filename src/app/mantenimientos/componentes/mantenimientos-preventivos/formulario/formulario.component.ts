...
import { ModalRegistroPreventivoComponent } from '../modal-registro/modal-registro.component';
import { ModalHistorialPreventivoComponent } from '../modal-historial/modal-historial.component';
...
export class FormularioPreventivoComponent implements OnInit {
  @ViewChild('modalRegistroPreventivo') modalRegistro!: ModalRegistroPreventivoComponent
  @ViewChild('modalHistorialPreventivo') modalHistorial!: ModalHistorialPreventivoComponent
  ...
  listarRegistros() {
    this.listaResgitros = []
    this.servicioMantenimientos.listarRegistros(1, this.usuario?.usuario, this.rol?.id).subscribe({
      next: (respuesta: any) => {
      ...
      error: (error: any) => {
        if (error.status === 500) {
          ...
          this.verificador = true; // Cambia el valor a true si no hay registros
  ...
  volver() {
    this.router.navigate(['/administrar/formulario-preventivo']);
  }
}
